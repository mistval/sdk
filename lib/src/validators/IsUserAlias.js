...
(function (UserRefValidationResult) {
    ...
    UserRefValidationResult[UserRefValidationResult["INVALID_ETH_USER_ALIAS"] = 2] = "INVALID_ETH_USER_ALIAS";
    UserRefValidationResult[UserRefValidationResult["INVALID_TON_USER_ALIAS"] = 3] = "INVALID_TON_USER_ALIAS";
    UserRefValidationResult[UserRefValidationResult["INVALID_FORMAT"] = 4] = "INVALID_FORMAT";
})(UserRefValidationResult || (UserRefValidationResult = {}));
const customMessages = {
    [UserRefValidationResult.INVALID_ETH_USER_ALIAS]: "User alias starting with 'eth|' must end with valid checksumed eth address.",
    [UserRefValidationResult.INVALID_TON_USER_ALIAS]: "User alias starting with 'ton|' must end with valid bounceable base64 TON address."
};
...
function validateUserAlias(value) {
    ...
    if (parts.length === 2) {
        ...
        if (parts[0] === "eth") {
            if (utils_1.signatures.isChecksumedEthAddress(parts[1])) {
                return UserRefValidationResult.VALID_USER_ALIAS;
            }
            else {
...
let IsUserAliasConstraint = class IsUserAliasConstraint {
    ...
    defaultMessage(args) {
        ...
        if (Array.isArray(value)) {
            const invalidValues = value.filter((val) => validateUserAlias(val) !== UserRefValidationResult.VALID_USER_ALIAS &&
